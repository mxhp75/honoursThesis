...
# Import the mirbase chromosome information
...
ggplot(data=rawNonZero, aes(x=samplename, y=rawmiRNA, fill = samples$group)) +
  ...
  geom_text(aes(5, 1600, label = "1571 Total miRNA"), size = 4, colour = "black") +
...
# remove low expressed miRNAs logical eg cpm >1 in >=20 samples (7 samples in the smallest group)
...
col <- brewer.pal(nsamples, "Paired")
par(mfrow=c(1,2))
plot(density(rawlcpm_plasma[,1]), col=col[1], lwd=2, ylim=c(0,0.15), las=2,
  main="", xlab="")
...
ggplot(data=filteredNonZero, aes(x=rowname, y=V1, fill=samples$group)) +
  ...
  geom_text(aes(5, 560, label = "542 Total miRNA"), size = 4, colour = "black") +
  scale_fill_discrete(name = "Trimester")
#dev.off()
...
expressionPlasmaTop20 <- expressionPlasma[order(rowMeans(expressionPlasma), decreasing = TRUE), ] %>%
  ...
  select(., -matches("PAC"))
expressionPlasmaTop20$Tissue <- "Placenta"
...
## unique miRs by fetal sex (counts filtered to exclude miRs w/ < 2 samples per week showing expression)

```{r warning=FALSE, error=FALSE}


sampleSex <- samples[, c(1, 9)] # select on the samplename and gestation (week) columns from the samples df

# new df of un-normalised counts
countsBySex <- t(countsPlasma) %>%
  as.data.frame() %>%
  tibble::rownames_to_column() # create a new transformed df of un-normalised counts with samplename as the first column

countsBySex <- left_join(sampleSex, countsBySex, by = c("samplename" = "rowname")) # left join the samples and counts data
countsBySex <- countsBySex[, -1] # remove unwanted samplename column


countsBySex <- aggregate(countsBySex, by = list(countsBySex$Sex), FUN = mean)[, -2] %>%
  rename(c('Group.1' = 'rowname')) %>%
  tibble::column_to_rownames() # aggregate the counts df such that each week now only has a single observation per miR

countsBySex <- t(countsBySex) # new counts df all finished with weeks as column headers - counts are means within weeks

# Heat Map of binary expression by fetal sex - all non-zero counts converted to 1.
countsBySex[which(countsBySex != 0)] <- 1

# new df all miRs for each fetal sex 
Female <- countsBySex[, "Female"] %>%
  as.data.frame() %>%
  tibble::rownames_to_column() %>%
  set_colnames(c("miRNA", "Female"))
Female <- Female[!(Female$Female==0),]

Male <- countsBySex[, "Male"]  %>%
  as.data.frame() %>%
  tibble::rownames_to_column() %>%
  set_colnames(c("miRNA", "Male"))
Male <- Male[!(Male$Male==0),]

# miRNAs with ubiquitous expression (expressed in both male and female)
ubiquitous <- countsBySex[ rowSums(countsBySex)==2, ] %>%
  as.data.frame()

# miRNAs with sex specific expression (expressed in either male or female, but not both)
sexSpecific <- countsBySex[ rowSums(countsBySex)==1, ] %>%
  as.data.frame()

pheatmap(sexSpecific[1:60,], 
         cluster_cols = FALSE,
         cluster_rows = TRUE,
         cellwidth = 15, 
         cellheight = 9,
         ...
         border_color = "white",
         treeheight_row = 0,
         labels_col = c("Female", "Male"),
         main="Binary Expression in\n Maternal Plasma by Fetal Sex")

# simple df of miRNA expressed only in male samples
maleOnly <- sexSpecific[, "Male", drop=FALSE] %>%
  as.data.frame() %>%
  set_colnames("Male")
# remove miRNAs with zero counts in all samples
maleOnly <- maleOnly[ rowSums(maleOnly)!=0, , drop = FALSE] %>%
  as.data.frame()

# simple df of miRNA expressed only in female samples
femaleOnly <- sexSpecific[, "Female", drop=FALSE] %>%
  as.data.frame() %>%
  set_colnames("Female")
# remove miRNAs with zero counts in all samples
femaleOnly <-femaleOnly[ rowSums(femaleOnly)!=0, , drop = FALSE] %>%
  as.data.frame()
