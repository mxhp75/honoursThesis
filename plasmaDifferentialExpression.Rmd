---
title: "plasmaDifferentialExpression"
...
# initial library size in plasma - no fill
...
# final library size - trimester fill
...
# final library size - no fill
#pdf("robustmiRNAPlasma.pdf")
...
#pdf("mds_plasma.pdf", height = 6, width = 11)
plotMDS(cpm(DGEList_plasma, log = TRUE), col = col.group, pch = 20, cex = 1.5)
...
#pdf("mds_plasma3.pdf", height = 6, width = 11)
plotMDS(cpm(DGEList_plasma, log = TRUE), labels = samples$gestation, cex = 1.5)
...
topTable <- topTable(fit, coef = 1, n = 10, sort = "p" , p = 0.05, adjust.method = "fdr") # requires eBayes.
...
ggplot(data = d4, aes(x = logFC, y = neg.log10FDR, colour = V1)) +
  ...
  #geom_hline(yintercept=1.321393)
#dev.off()
...
fit <- eBayes(fit) # moderated t-test
summary(decideTests(fit))
topTable <- topTable(fit, coef = 1, n = 10, sort = "p" , adjust.method = "fdr") # requires eBayes.
plotMD(fit)
