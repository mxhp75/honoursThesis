...
### Trying  new design here but don't really understand it yet
...
testdesign <- model.matrix(~ oxygenation + Sex + group, data = samples)
testcont.mat <- c(0, 1, 0, 0) 
# position 1=intercept, position 2=group, position 3=sex
# when looking at trimester differences c(0,1,0)
# when looking at sex differences c(0,0,1)

testv <- voomWithQualityWeights(DGEList_placenta, testdesign, plot = TRUE)

testfit <- lmFit(testv, testdesign)
testfit <- contrasts.fit(testfit, testcont.mat)
testfit <- eBayes(testfit) # moderated t-test
summary(decideTests(testfit))
testtopTable <- topTable(testfit, coef = 1, n = Inf, sort = "p" , p = 0.05, adjust.method = "fdr") # requires eBayes.
plotMD(testfit, main = "Placenta: Mean Difference Plot\nTrimester")
